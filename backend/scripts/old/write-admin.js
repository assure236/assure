...
write('controllers/kycController.js', `const { User, Document } = require('../models');
const path = require('path');
const fs = require('fs');
...
exports.getKycStatus = async (req, res, next) => {
  try {
    ...
    const user = await User.findById(userId).select('kyc_status kyc_rejection_reason full_name email mobile');
    const documents = await Document.find({ user_id: userId }).sort({ created_at: -1 });
    res.json({ success: true, data: { kyc_status: user.kyc_status, rejection_reason: user.kyc_rejection_reason, documents, user: { full_name: user.full_name, email: user.email, mobile: user.mobile } } });
  } catch (err) { next(err); }
};
...
write('routes/adminRoutes.js', `const express = require('express');
...
const superAdminOnly = [authMiddleware, authorizeRoles('admin')];
...
router.get('/dashboard', adminOnly, async (req, res, next) => {
  try {
    ...
    const [totalMembers, activeGroups, pendingKYC, openTickets] = await Promise.all([
    ...
    const activeAuctions = await Auction.find({ status: { $in: ['scheduled', 'in_progress'] } })
    ...
    res.json({
      success: true,
      data: {
        stats: { totalMembers, activeGroups, monthlyRevenue: revenueAgg[0]?.total || 0, totalRevenue: totalRevenueAgg[0]?.total || 0, pendingKYC, openTickets, activeAuctions: activeAuctions.length },
        monthlyCollections, recentMembers, activeAuctions, pendingDisbursals,
      }
    });
  } catch (err) { next(err); }
});
...
router.get('/users/:id', adminOnly, async (req, res, next) => {
  try {
    ...
    const [memberships, documents, payments] = await Promise.all([
      ChitMember.find({ user_id: user._id }).populate('chit_group_id', 'group_name chit_value status'),
    ...
    res.json({ success: true, data: { user, memberships, documents, recentPayments: payments } });
  } catch (err) { next(err); }
});
...
router.post('/auctions', adminOnly, async (req, res, next) => {
  try {
    ...
    res.status(201).json({ success: true, message: 'Auction scheduled', data: auction });
  } catch (err) { next(err); }
});
...
router.post('/auctions/:id/start', adminOnly, async (req, res, next) => {
  try {
    const auction = await Auction.findByIdAndUpdate(req.params.id, { status: 'in_progress' }, { new: true }).populate('chit_group_id', 'group_name');
    if (!auction) return res.status(404).json({ success: false, message: 'Not found' });
    const io = req.app.get('io');
    if (io) io.to('auction:' + auction._id).emit('auction_started', { auction_id: String(auction._id), group_name: auction.chit_group_id?.group_name });
...
router.post('/auctions/:id/end', adminOnly, async (req, res, next) => {
  try {
    const auction = await Auction.findById(req.params.id);
    if (!auction) return res.status(404).json({ success: false, message: 'Not found' });
    const winningBid = await Bid.findOne({ auction_id: auction._id }).sort({ bid_amount: -1 });
    if (!winningBid) return res.status(400).json({ success: false, message: 'No bids placed' });
    await Auction.findByIdAndUpdate(auction._id, {
      status: 'completed',
      winner_id: winningBid.user_id,
      winning_bid_amount: winningBid.bid_amount,
      dividend_amount: winningBid.bid_amount,
      actual_end_time: new Date(),
    });
    await ChitMember.findOneAndUpdate({ chit_group_id: auction.chit_group_id, user_id: winningBid.user_id }, { has_won_auction: true });
    const updated = await Auction.findById(auction._id).populate('winner_id', 'full_name mobile');
    const io = req.app.get('io');
    if (io) io.to('auction:' + auction._id).emit('auction_ended', { auction_id: String(auction._id), winner_name: updated.winner_id?.full_name, winning_amount: winningBid.bid_amount });
    if (updated.winner_id?.mobile) notificationService.sendSMS(updated.winner_id.mobile, 'Congratulations! You won the auction with bid ₹' + winningBid.bid_amount + '. Disbursement in 2-3 days. - Assure ChitFunds').catch(() => {});
...
router.get('/disbursals', adminOnly, async (req, res, next) => {
  try {
    ...
    const total = await Auction.countDocuments(filter);
    const disbursals = await Auction.find(filter)
...
router.post('/payments/record', adminOnly, async (req, res, next) => {
  try {
    ...
    const payment = await Payment.create({
      ...
      payment_gateway: 'Manual', payment_status: 'success', payment_date: new Date(), admin_notes: notes,
    });
...
router.post('/communications/send', adminOnly, async (req, res, next) => {
  try {
    const { user_ids, type, subject, message, send_to_all } = req.body;
    ...
    for (const user of targets) {
      try {
        if (type === 'sms' && user.mobile) {
          ...
          await CommunicationLog.create({ user_id: user._id, type: 'sms', subject, message, status: 'sent', sent_by: adminId });
          sent++;
          ...
          await CommunicationLog.create({ user_id: user._id, type: 'email', subject, message, status: 'sent', sent_by: adminId });
          sent++;
        } else {
          await Notification.create({ user_id: user._id, title: subject || 'Notification', message, type: 'general', is_read: false });
          await CommunicationLog.create({ user_id: user._id, type: 'notification', subject, message, status: 'sent', sent_by: adminId });
          sent++;
        }
      } catch (e) {
        failed++;
        await CommunicationLog.create({ user_id: user._id, type, subject, message, status: 'failed', sent_by: adminId, error_message: e.message });
      }
    }
...
console.log('\nAdmin routes and KYC controller written!');
